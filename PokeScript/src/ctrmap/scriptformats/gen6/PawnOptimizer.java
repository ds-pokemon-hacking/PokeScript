...
public class PawnOptimizer {
	...
	public static OptimizePattern[] patterns = new OptimizePattern[]{
		...
		PawnInstruction.Commands.CONST_PRI, PawnInstruction.Commands.PUSH_PRI
		),
		new OptimizePattern(
		...
		PawnInstruction.Commands.CONST_PRI, PawnInstruction.Commands.STOR_P_S_PRI
		),
		new OptimizePattern(
		...
		PawnInstruction.Commands.LOAD_S_PRI, PawnInstruction.Commands.MOVE_ALT
		),
		new OptimizePattern(
		...
		PawnInstruction.Commands.PUSH_S, null, PawnInstruction.Commands.PPRI
		),
		new OptimizePattern(
		...
		PawnInstruction.Commands.PUSH_S, PawnInstruction.Commands.PPRI
		),
		new OptimizePattern(
		"newins jzer JZER;jmptransfer 1 jzer;append jzer;remapall jzer;deleteall",
		PawnInstruction.Commands.NOT, PawnInstruction.Commands.JNZ
		),
		new OptimizePattern(
		"newins jnz JNZ;jmptransfer 1 jnz;append jnz;remapall jnz;deleteall",
		PawnInstruction.Commands.NOT, PawnInstruction.Commands.JZER
		),
		new OptimizePattern(
		"newins jsgeq JSGEQ;jmptransfer 1 jsgeq;append jsgeq;remapall jsgeq;deleteall",
		PawnInstruction.Commands.SLESS, PawnInstruction.Commands.JZER
		),
		new OptimizePattern(
		"newins jsgrtr JSGRTR;jmptransfer 1 jsgrtr;append jsgrtr;remapall jsgrtr;deleteall",
		PawnInstruction.Commands.SLEQ, PawnInstruction.Commands.JZER
		),
		new OptimizePattern(
		...
		PawnInstruction.Commands.LOAD_S_PRI, PawnInstruction.Commands.CONST_ALT, PawnInstruction.Commands.ADD, PawnInstruction.Commands.STOR_S_PRI
		),
		new OptimizePattern(
		...
		PawnInstruction.Commands.CONST_ALT, PawnInstruction.Commands.LOAD_S_PRI, PawnInstruction.Commands.ADD, PawnInstruction.Commands.STOR_S_PRI
		),
		new OptimizePattern(
		"getarg stack 0 0; isarg stack 0; remapallnext; deleteall",
		PawnInstruction.Commands.STACK
		),
		new OptimizePattern(
		...
		PawnInstruction.Commands.CONST_PRI, PawnInstruction.Commands.NEG
		),
		new OptimizePattern(
		...
		PawnInstruction.Commands.PUSH_C, PawnInstruction.Commands.PUSH_C, PawnInstruction.Commands.PUSH_C, PawnInstruction.Commands.PUSH_C, PawnInstruction.Commands.PUSH_C
		),
		new OptimizePattern(
		...
		PawnInstruction.Commands.PUSH_C, PawnInstruction.Commands.PUSH_C, PawnInstruction.Commands.PUSH_C, PawnInstruction.Commands.PUSH_C
		),
		new OptimizePattern(
		...
		PawnInstruction.Commands.PUSH_C, PawnInstruction.Commands.PUSH_C, PawnInstruction.Commands.PUSH_C
		),
		new OptimizePattern(
		...
		PawnInstruction.Commands.PUSH_C, PawnInstruction.Commands.PUSH_C
		)
	};
	...
	public static class OptimizePattern {
		...
		private PawnInstruction.Commands[] cmds;
		...
		public OptimizePattern(String filter, PawnInstruction.Commands... pattern) {
			this(filter, false, pattern);
		}

		public OptimizePattern(String filter, boolean strict, PawnInstruction.Commands... pattern) {
			this.filter = filter;
			this.strict = strict;
			this.cmds = pattern;
		}
		...
		public boolean match(List<PawnInstruction> test, int position) {
			...
			if (position + cmds.length - 1 < test.size()) {
				for (int i = 0; i < cmds.length; i++) {
					...
					PawnInstruction.Commands fullCmd = test.get(position + i).getCommandE();
					if (fullCmd == cmds[i] || (!strict && fullCmd == PawnInstruction.getPackedEquiv(cmds[i]))) {
						yesno = true;
					} else {
						yesno = false;
						break;
		...
		public boolean optimize() {
			...
			Map<String, Integer> numberBuf = new HashMap<>();
			...
			for (String c : ptn) {
				...
				switch (args[0]) {
					case "newins":
						...
						PawnInstruction.Commands opcodeC = null;
						for (PawnInstruction.Commands op : PawnInstruction.cmdList) {
					...
					case "getarg":
						...
						break;
					case "setarg":
						insBuf.get(args[2]).argumentCells[Integer.parseInt(args[3])] = numberBuf.get(args[1]);
						break;
					case "arg2ptr":
						...
						break;
					case "getptr":
						numberBuf.put(args[1], insBuf.get(args[2]).pointer);
						break;
					case "append":
		...
		for (PawnInstruction i : tgt) {
			for (PawnInstruction.JumpListener jl : i.jmpListeners) {
				if (jl instanceof PawnInstruction.CaseListener) {
					...
					for (Map.Entry<Integer, PawnInstruction> t : cl.targets.entrySet()) {
						if (toRemap.contains(t.getValue())) {
							t.setValue(target);
						}
					}
				} else {
					if (toRemap.contains(jl.getParent())) {
						jl.setParent(target);
					}
				}
			}
