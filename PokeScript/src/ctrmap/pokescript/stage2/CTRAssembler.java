...
public class CTRAssembler {
	...
	private static String getHashableName(String name) {
		int lio = name.lastIndexOf('.');
		if (lio != -1) {
			return name.substring(lio + 1);
		}
		return name;
	}
	...
	public static void assemble(NCompileGraph graph, GFLPawnScript target) {
		...
		target.mainEntryPointDummy = null;
		...
		if (main != null) {
			for (NCompilableMethod m : graph.methods) {
				if (!m.def.isResident && m.def.timesUsed == 0 && !m.hasModifier(Modifier.PUBLIC) && m != main) {
		...
		for (Variable v : graph.globals.variables) {
			...
			if (g.hasModifier(Modifier.PUBLIC)) {
				target.publicVars.add(new PawnPrefixEntry(8, PawnPrefixEntry.Type.PUBLIC_VAR,
					g.getPointer(graph),
					target.hashName(getHashableName(g.name))
				));
			}
		...
		for (AInstruction i : allInstructions) {
			...
			for (ACompiledInstruction ci : compiled) {
				if (ci == null) {
					throw new RuntimeException("Invalid NULL instruction. What the fuck?");
				}
		...
		for (NCompilableMethod method : graph.methods) {
			if (method.hasModifier(Modifier.NATIVE)) {
				String nativeName = method.def.name;
				if (method.def.extendsBase != null) {
					nativeName = method.def.extendsBase;
				}
				if (nativeName.contains(".")) {
					//remove namespace
					nativeName = nativeName.substring(nativeName.lastIndexOf(".") + 1);
				}
				PawnPrefixEntry natv = new PawnPrefixEntry(8, PawnPrefixEntry.Type.NATIVE,
					0x00000000,
					target.hashName(nativeName)
				);
		...
		for (NCompilableMethod m : graph.methods) {
			if (m.hasModifier(Modifier.PUBLIC) && !m.hasModifier(Modifier.NATIVE) && m != main) {
				target.publics.add(new PawnPrefixEntry(8, PawnPrefixEntry.Type.PUBLIC,
					m.getPointer(),
					target.hashName(getHashableName(m.def.name))
				));
			}
		}
